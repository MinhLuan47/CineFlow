...
const mapNormalizedToRankMovie = (normalized: NormalizedMovie): RankMovie => {
  return {
    id: normalized.id,
    ...
    views: Math.round(normalized.popularity * 150),
  };
};
...
export const TopRanking: React.FC = () => {
  const { data: movies, loading, error, refetch } = usePopularMovies();

  const rankedMovies = React.useMemo(() => {
    if (!movies) return [];
    return movies.map(mapNormalizedToRankMovie).slice(0, 10);
  }, [movies]);
  ...
  const renderRankItem = (movie: RankMovie, index: number) => {
    const rank = index + 1;
    const isTop3 = rank <= 3;
